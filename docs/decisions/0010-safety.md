...
- **Admin** uses `AdminUser` + `AdminSession` and `tessera_admin_*` cookies with JWT issuer `tessera-admin`, signed with `JWT_ADMIN_SECRET` rather than the user access secret. Roles: moderator, admin, superadmin. Every action writes `AuditLog`.
