...
export const adminApi = {
  ...
  act: (id: string, kind: string, note: string) =>
    request(`/v1/admin/cases/${id}/action`, { method: 'POST', body: JSON.stringify({ kind, note }) }),
  ...
  suspend: (id: string, reason: string, days: number | null) =>
    request(`/v1/admin/users/${id}/suspend`, { method: 'POST', body: JSON.stringify({ reason, days }) }),
  ...
  resolveAppeal: (id: string, status: 'upheld' | 'rejected', decision: string) =>
    request(`/v1/admin/appeals/${id}/resolve`, { method: 'POST', body: JSON.stringify({ status, decision }) }),
  audit: () => request('/v1/admin/audit'),
  keywords: () => request('/v1/admin/keyword-filters'),
  addKeyword: (keyword: string, action: string) =>
    request('/v1/admin/keyword-filters', { method: 'POST', body: JSON.stringify({ keyword, action }) }),
